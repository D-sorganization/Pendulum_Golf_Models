...
class PinocchioDoublePendulum:
    ...
    def __init__(self, params: DoublePendulumParameters | None = None) -> None:
        ...
        if pin is None:
            raise ImportError("Pinocchio is not installed. Please install it to use this class.")
    ...
    def bias_forces(self, state: DoublePendulumState) -> np.ndarray:
        ...
        return pin.rnea(self.model, self.data, q, v, np.zeros(self.model.nv))
    ...
    def forward_dynamics(self, state: DoublePendulumState, tau: np.ndarray) -> np.ndarray:
        """Compute joint accelerations qddot = M^-1 (tau - h(q, v))."""
        q, v = self._get_q_v(state)
        return pin.aba(self.model, self.data, q, v, tau)

    def inverse_dynamics(self, state: DoublePendulumState, acc: np.ndarray) -> np.ndarray:
        """Compute required torques tau = M qddot + h(q, v)."""
        q, v = self._get_q_v(state)
        return pin.rnea(self.model, self.data, q, v, acc)

    def total_energy(self, state: DoublePendulumState) -> float:
        """Compute total energy (Kinetic + Potential)."""
        q, v = self._get_q_v(state)
        pin.computeAllTerms(self.model, self.data, q, v)
        return self.data.kinetic_energy + self.data.potential_energy
